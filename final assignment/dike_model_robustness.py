...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        pd.DataFrame(scenarios.designs, columns=scenarios.params).to_csv(os.path.join('experiment', 'scenarios_%d_%d_%d.csv' % (n_scenarios, nfe, i)), index=False)
        # write policies to csv
        robust_results.to_csv(os.path.join('experiment', 'robust_policies_%d_%d_%d.csv' % (n_scenarios, nfe, i)), index=False)
